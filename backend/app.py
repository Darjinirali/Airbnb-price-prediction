...
Fixed Version (CORS + Auth + Register/Login + Model)
"""
...
# Cross-site session cookies (Vercel frontend + Render backend)
...
CORS(
    ...
    origins=[
        ...
        "http://localhost:5175",
        "http://localhost:3000",
        "https://airbnb-price-prediction-sigma.vercel.app"
    ]
)
...
REAL_DATA  = os.path.join(DATA_DIR, 'real_data.csv')
...
def generate_dummy_data(n=1000) -> pd.DataFrame:
    ...
    for _ in range(n):
        ...
        amen = list(np.random.choice(amenities_options))
...
def graphs():
    try:
        ...
        return jsonify({
            'city_comparison': [{'city': k, 'price': v} for k, v in city_prices.items()],
